...
plot(ir.pca,ir.pcb,main="PC1 and PC2 for Ionosphere",xlab="x", ylab="y", pch=19)

#4.2
ir.pcaa <-(prcomp(MyData1,center = FALSE,scale. = FALSE)$sdev)^2
plot(ir.pcaa,type='o',main="Ionosphere dataset",xlab = "Principal_Component" ,ylab = "Variance")
...
MyData<-as.matrix(prcomp(MyData1,center = FALSE,scale. = FALSE)$rotation[,1:15])
...
avg_error2_em<-matrix(0,nrow = 5,ncol=21)
iter_em<-matrix(0,nrow = 5,ncol=20)
#MyData
...
#boxplot(avg_error2_em[,5]~avg_error2_em[,1],pch=20,xlab ="K" ,ylab = "Total Error",cex=0.5, main="K-Means on Ionosphere dataset")
###################################################################################################################################
####################K-MEANS#####################################################
